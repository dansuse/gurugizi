...
<div id="container">
	...
	<?php
		...
		echo '<H1>Laporan Jawaban Nutritionist '.$nutritionist->nama.'</H1>';
		...
		echo '</p>';
		echo '<p>';
		echo form_submit('btnRefreshJawaban','Refresh');
		echo '</p>';
		echo '<hr>';
		...
		foreach($jawaban as $p)
		{
			...
			echo $p->isi_jawaban;
			echo '</p>';
			echo '<p><b>Tanggal Jawaban : </b>';
			echo $p->waktu_jawab;
			echo '</p>';
			
			$convertEmail = str_replace("@", ':', $p->email_nutritionist);
			echo '<p>';
			echo '<a style="text-decoration:none; color:black;" href = "'.site_url('c_laporan/deleteJawaban/'.$convertEmail.'/'.$p->id_jawaban).'"><input type = "button" value = "Hapus Jawaban">'.'</input></a>';
			echo '</p>';
