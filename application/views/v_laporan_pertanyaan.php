...
<div id="container">
	...
	<?php
		...
		echo '<H1>Laporan Pertanyaan Pasien '.$pasien->nama.'</H1>';
		echo form_submit('btnKembaliKeLapPasien','Kembali Ke Laporan Pasien');
		echo '<HR>';
		...
		echo '</p>';
		echo '<p>';
		echo form_submit('btnRefreshPertanyaan','Refresh');
		echo '</p>';
		echo '<hr>';
		...
		{
			echo '<H4>';
			echo $p->judul_pertanyaan;
			echo '</H4>';
			echo '<p><b>Isi : </b><br>';
			echo $p->isi_pertanyaan;
			echo '</p>';
			echo '<p><b>Tag : </b><br>';
			echo $p->tag_pertanyaan;
			echo '</p>';
			echo '<p><b>Tanggal Pertanyaan : </b>';
			echo $p->waktu_bertanya;
			echo '</p>';
			
			$convertEmail = str_replace("@", ':', $p->email_pasien);
			echo '<p>';
			echo '<a style="text-decoration:none; color:black;" href = "'.site_url('c_laporan/deletePertanyaan/'.$convertEmail.'/'.$p->id_pertanyaan).'"><input type = "button" value = "Hapus Pertanyaan">'.'</input></a>';
			echo '</p>';
